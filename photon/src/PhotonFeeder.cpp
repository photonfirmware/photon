...
// NEVER change this line manually, any version other than beta should ONLY be generated by the CI
#define VERSION beta
// --------------------
...
void PhotonFeeder::showVersion() {
    ...
    if(VERSION.major == 0){ // flashing red, beta
    ...
    else if (VERSION.major == 1){ // flashing green, v1.X.X
...
void PhotonFeeder::vendorSpecific(uint8_t options[]) {
    uint8_t byte_0 = options[0];

    int blue = byte_0 & 1;
    int green = (byte_0 >> 1) & 1;
    int red = (byte_0 >> 2) & 1;
    int set = (byte_0 >> 3) & 1;
    ...
    // TODO return firmware version based on VERSION define
}
